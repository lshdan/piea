...
class Filter:
  ...
  # dxy: generate the parameters for this filter
  ...
  # dxy: this is the function to use filter.
  ...
  def apply(self,
    ...
    # dxy: !!! important: process the filter.
    # dxy: lerp is linear interpolation, which is used for the mask
    # dxy: in all these experiments, the mask is disabled.
...
class ExposureFilter(Filter):
  ...
  # dxy: initialize the parameter (sized 1)
  # dxy: normalize the features to [-exposure_range,+exposure_range] which is [-3.5,3.5] in example
  def filter_param_regressor(self, features):
    return tanh_range(
